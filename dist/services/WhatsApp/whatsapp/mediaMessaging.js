...
exports.sendImagesGroup = sendImagesGroup;
...
async function sendImagesGroup(config, to, imageUrls, options = {}) {
    const responses = [];
    const delay = options.delayBetweenImages || 300;
    for (let i = 0; i < imageUrls.length; i++) {
        if (i > 0) {
            await new Promise(resolve => setTimeout(resolve, delay));
        }
        const response = await sendImage(config, to, imageUrls[i], undefined, {
            replyToMessageId: i === 0 ? options.replyToMessageId : undefined
        });
        responses.push(response);
    }
