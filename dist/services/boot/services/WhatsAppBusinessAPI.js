...
async function sendWhatsAppTemplateCampaign(campaignId) {
    ...
    try {
        ...
        const results = await sendMessagesInBatches(clientsToSend.slice(0, actualTargets), campaign.data);
    ...
    try {
        ...
        switch (audienceType) {
            ...
            case 'inactive':
                ...
                break;
            default:
                const selectedIds = campaign.clients?.map((client) => client.id) || [];
...
async function sendSingleMessage(client, campaign) {
    ...
    try {
        const parameters = (campaign.template.variables || []).map((variable) => {
