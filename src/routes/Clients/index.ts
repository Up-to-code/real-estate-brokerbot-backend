...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (
      updates.phoneNumber &&
      updates.phoneNumber !== existingClient.phoneNumber
    ) {
      const duplicateClient = await prisma.client.findUnique({
    ...
    for (const clientData of clients) {
      try {
        ...
        if (!clientData.name || !clientData.phoneNumber) {
          results.failed++;
          results.errors.push(
            `Client missing required fields: ${JSON.stringify(clientData)}`
          );
          continue;
        }
        ...
        if (existingClient) {
          results.failed++;
          results.errors.push(
            `Duplicate phone number: ${clientData.phoneNumber}`
          );
          continue;
        }
        ...
        results.failed++;
        results.errors.push(
          `Error creating client ${clientData.name}: ${error.message}`
        );
      }
    }
